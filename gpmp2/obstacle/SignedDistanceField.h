...
public:
  ...
  double getSignedDistance_zhjd(const gtsam::Point3& point) const {
    const float_index pidx = convertPoint3toCell(point);
    return signed_distance(pidx);
  }
  double getSignedDistance_zhjd(const gtsam::Point3& point, gtsam::Vector3& g) const {
    const float_index pidx = convertPoint3toCell(point);
  ...
  inline double getSignedDistance(const gtsam::Point3& point) const {
    const float_index pidx = convertPoint3toCell(point);
    // std::cout<<"[inline]:idx "<<pidx.get<0>()<<","<<pidx.get<1>()<<","<<pidx.get<2>()<<","<<std::endl;
    // std::cout<<"[inline]:signed_distance "<<signed_distance(pidx)<<std::endl;
    return signed_distance(pidx);
  }
  ...
  inline double signed_distance(const float_index& idx) const {
    ...
    // std::cout <<"[inline 8 signed_distance]: "<<signed_distance(lri, lci, lzi)<<","
    ...
    //           <<signed_distance(hri, hci, hzi)<<std::endl;
