...
public:
  ...
  gtsam::Vector evaluateError(
    ...
    if (H1) {
      ...
      Matrix33 H_er;
      //张嘉东
      // curr_orientation： 基座到目标约束关节的变换矩阵的旋转部分
      // H_rp，我猜测是，机械臂末端在xyz轴上的旋转量，对末端的旋转和平移量  的偏导
      Rot3 curr_orientation = joint_pos[joint_].rotation(H_rp);  
      
      // 张嘉东    为什么第一行的偏导是位移量和旋转量的相加
      // 根据maltab程序，joint_等于 arm.dof()-1 
      // des_orientation_ is desired workspace orientation for joint
