...
void RobotModel<FK>::sphereCenters(const Pose& jp,
  ...
  for (size_t sph_idx = 0; sph_idx < nr_body_spheres(); sph_idx++) {
    if (J_point_conf) {
      ...
      // body_spheres_ 是所有的球
      // body_spheres_[sph_idx]  第sph_idx个球
      // body_spheres_[sph_idx].link_id   球对应的关节的索引
      // link_poses[body_spheres_[sph_idx].link_id]     第sph_idx个球对应的关节的位姿

      // body_spheres_[sph_idx].center    球在对应的关节坐标系中的坐标
