...
void Arm::forwardKinematics(
    /* 输入 */  const Vector& jp  /* 各关节的角度 */, boost::optional<const Vector&> jv,
    /* 输出 */  std::vector<gtsam::Pose3>& jpx  /* 各关节的坐标 */, boost::optional<std::vector<gtsam::Vector3>&> jvx,
  ...
  for (size_t i = 1; i <= dof(); i++) {
    ...
    // H[i-1] 表示从第 i-1 个关节到第 i 个关节的变换矩阵。
    // getH 函数是一个成员函数，用于计算第 i-1 个关节在给定关节角度下的变换矩阵。
    // 在这里，jp(i-1) 表示机器人在第 i-1 个关节的关节角度。
  ...
  for (size_t i = 0; i < dof(); i++) {
    ...
    //最重要的位置： Find end effector points
    jpx[i] = Pose3(Rot3(Ho[i+1].block<3,3>(0,0)), Point3(Ho[i+1].col(3).head<3>()));

    //速度，目前还用不到 and end effector velocities
    if (jv && jvx)
      (*jvx)[i] = J[i] * (*jv);
